...
function BudgetCard({ cat, budget, spent, onEdit, onRemove }) {
  ...
  return (
    ...
    >
      <Card className={`transition-all ${isOver ? 'ring-2 ring-[--danger-border] ring-offset-2 ring-offset-[--bg-app]' : ''}`}>
        {/* Cabeçalho */}
        ...
        {budget ? (
          <>
            ...
            {/* Botão editar */}
            <button
              onClick={() => onEdit({ categoryId: cat.id, amount: budget.amount, budgetId: budget.id })}
  ...
  return (
    <div className="space-y-5 pb-24 lg:pb-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <Button variant="primary" size="sm" icon={<Plus />} onClick={() => handleEdit({ categoryId: '', amount: '' })}>
          Definir orçamento
        </Button>
      </div>
      ...
      {expenseCategories.length === 0 ? (
        <EmptyState icon={<PieChart />} title="Nenhuma categoria de despesa"
