          ...
          <div>
            ...
            <div className="flex gap-2 mt-2">
              {DEFAULT_COLORS.map(c => (
                <button key={c} onClick={() => setForm(f => ({ ...f, color: c }))} className={`w-8 h-8 rounded-full border-2 ${form.color === c ? 'border-white ring-2 ring-[--brand-500]' : 'border-transparent'}`} style={{ background: c }} />
              ))}
            </div>
          </div>
          <div>
            <label className="text-sm font-medium text-[--text-secondary]">Tipo</label>
            <select value={form.type} onChange={e => setForm(f => ({ ...f, type: e.target.value }))} className="w-full mt-1 bg-[--bg-surface] border border-[--border-default] rounded-xl px-4 py-2">
