    ...
    <AnimatePresence>
      <motion.div
        ...
        <motion.div
          ...
          className="relative z-10 w-full max-w-sm mx-4 mb-6 sm:mb-0 bg-[--bg-card] border border-[--border-glow] rounded-3xl shadow-2xl overflow-hidden">
          ...
          <div className="p-6">
            ...
            <button onClick={finish}
              className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-[--bg-hover] text-[--text-tertiary] hover:text-[--text-secondary] transition-colors">
              <X size={16} />
            </button>
            ...
            {/* Conteúdo */}
            <h3 className="text-lg font-black text-[--text-primary] mb-2">{current.title}</h3>
              ...
              {STEPS.map((_, i) => (
                <div key={i}
                  className={`h-1.5 rounded-full transition-all duration-300 ${
                    i === step ? 'w-6 bg-[--brand-600]' : i < step ? 'w-3 bg-[--brand-400]' : 'w-3 bg-[--bg-hover]'
                  }`} />
              ))}
            </div>
            ...
            <div className="flex gap-2">
              {!isFirst && (
                <button onClick={prev}
                  className="flex items-center gap-1 px-4 py-2.5 rounded-xl border border-[--border-default] text-sm font-medium text-[--text-secondary] hover:bg-[--bg-hover] transition-colors">
            ...
            {!isFirst && (
              <button onClick={finish}
                className="w-full text-center text-xs text-[--text-tertiary] hover:text-[--text-secondary] mt-3 transition-colors">
                Pular tour
              </button>
            )}
          </div>
