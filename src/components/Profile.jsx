...
export default function Profile() {
  ...
  const handlePhotoChange = (e) => {
    ...
    if (file.size > 2 * 1024 * 1024) {
      toastErr('Imagem deve ter no máximo 2MB')
      return
    }
  ...
  const handleSaveProfile = async () => {
    if (!displayName.trim()) {
      toastErr('O nome não pode ficar em branco.')
      return
    }
    setSaving(true)
    try {
      ...
      if (photoBase64) {
        ...
        localStorage.setItem(`meu_real_photo_${currentUser.uid}`, photoBase64)
  ...
  const handleChangePassword = async () => {
    ...
    if (passForm.new.length < 6) {
      setPassError('Nova senha: mínimo 6 caracteres.')
      return
    }
  ...
  const avatarSrc =
    photoPreview ||
    localStorage.getItem(`meu_real_photo_${user?.uid}`) ||
            ...
            {status.isTrial && status.isPremium && (
              <div className="p-3 rounded-xl bg-[--brand-50] border border-[--brand-200]">
                ...
                <p className="text-xs text-[--brand-600] mt-0.5">
                  {status.daysLeft} dia{status.daysLeft !== 1 ? 's' : ''} restante
                  {status.daysLeft !== 1 ? 's' : ''} de experiência Premium gratuita.
                </p>
              </div>
            )}
            {!status.isPremium && (
              ...
              {PREMIUM_FEATURES.map((f) => (
                <div key={f} className="flex items-center gap-2">
                  <CheckCircle
                    size={13}
                    className={
                      status.isPremium ? 'text-[--success-icon]' : 'text-[--text-tertiary]'
                    }
                  />
        ...
        <div className="space-y-4">
          ...
          <div>
            <Input
              ...
              icon={<Mail size={15} />}
              disabled
              className="opacity-50 cursor-not-allowed select-none bg-[--bg-subtle]"
            />
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-xl bg-[--bg-subtle] border border-[--border-default] space-y-3">
            ...
            <div className="space-y-1.5 text-sm text-[--text-secondary]">
              ...
              <p>
                2. Transfira <strong className="text-[--text-primary]">{PIX_AMOUNT}</strong> para a
                chave:
              </p>
            </div>
          ...
          <p className="text-xs text-[--text-tertiary] text-center leading-relaxed">
            O plano vigora por 30 dias a partir da confirmação.
            <br />
