...
export function isReportDue(
  ...
  if (settings.frequency === 'weekly') {
    return (
      local.weekday === Number(settings.weekday || 1)
    )
  }
  ...
  if (settings.frequency === 'monthly') {
    return (
      local.day === Number(settings.monthDay || 1)
    )
  }
...
export function highestBudgetAlert({
  percentage,
...
export function goalAlertCandidates(
  goal,
  now = new Date(),
) {
  const result = []
